...
def data_gen(i):
    ...
    while True:
        if phase < 2*180:
            ...
            phase=0
        yield random.randint(0, 100), phase

def update(data):
    print(data)
    tw, phase = data
    title.set_text(u"|TW| = {}, Angle: {}°".format(tw, phase))
    arr1 = ax.arrow(np.deg2rad(phase), 0, 0, 10, alpha = 0.5, width = 0.080, edgecolor = 'red', facecolor = 'red', lw = 2, zorder = 5)
    return arr1,title,
